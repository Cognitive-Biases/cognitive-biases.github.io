...
const paths = ["/research/", "/data/", "/partners/", ...(researchNotes.entries || []).map((note) => `/research/${note.slug}/`)];
const sitemapPath = join(OUT, "sitemap.xml");
let sitemap = await readFile(sitemapPath, "utf8");
for (const path of paths) {
  const url = `${SITE}${path}`;
  if (sitemap.includes(`<loc>${url}</loc>`)) continue;
  sitemap = sitemap.replace("</urlset>", `<url><loc>${url}</loc><lastmod>${TODAY}</lastmod></url></urlset>`);
}
