...
const [release, canonicalBiasesRaw, canonicalTechniquesDoc, localizedTechniques, ui, pages, locales, aiLocales] = await Promise.all([
  readJson("data/release.json"), readJson("data/biases.json"), readJson("data/techniques.json"), readJson("data/techniques-es.json"),
  readJson("data/ui-es.json"), readJson("data/pages-es.json"), readJson("data/locales.json"), readJson("ai/locales.json")
]);
...
for (const entry of translations.entries) {
  ...
  seenIds.add(entry.canonicalId); seenSlugs.add(entry.localizedSlug);
  ...
  for (const field of ["localizedLabel", "englishLabel", "summary", "practicalQuestion", "boundary"]) if (!String(entry[field] || "").trim()) throw new Error(`${entry.canonicalId}: missing ${field}.`);
  ...
  if (/\b(?:TODO|TBD|placeholder)\b/i.test(JSON.stringify(entry))) throw new Error(`${entry.canonicalId}: Spanish record contains editorial placeholder text.`);
  for (const slug of entry.techniqueSlugs || []) if (!canonicalTechniqueBySlug.has(slug)) throw new Error(`${entry.canonicalId}: unknown canonical technique ${slug}.`);
...
for (const bias of publishedBiases) if (!seenIds.has(bias.slug)) throw new Error(`${bias.slug}: missing Spanish translation.`);
...
for (const entry of localizedTechniques.entries) {
  ...
  seenTechniques.add(entry.canonicalSlug); seenTechniqueSlugs.add(entry.localizedSlug);
  if (entry.state !== "reviewed") throw new Error(`${entry.canonicalSlug}: Spanish technique must be reviewed.`);
  for (const field of ["title", "whenToUse", "example", "whyItCanHelp", "limitations"]) if (!String(entry[field] || "").trim()) throw new Error(`${entry.canonicalSlug}: missing Spanish ${field}.`);
...
const expected = [
  ...
  ...translations.entries.map((entry) => ({ path: `/es/sesgos/${entry.localizedSlug}/`, en: `/biases/${entry.canonicalId}/`, fr: `/fr/biais/${frenchByCanonical.get(entry.canonicalId).localizedSlug}/` })),
  ...localizedTechniques.entries.map((entry) => {
    const frTech = null;
    return { path: `/es/tecnicas/${entry.localizedSlug}/`, en: `/techniques/${entry.canonicalSlug}/`, frCanonical: entry.canonicalSlug };
  })
];

await access("dist/es/index.html");
const sitemap = await readFile("dist/sitemap.xml", "utf8");
const frenchTechniques = await readJson("data/techniques-fr.json");
const frenchTechniqueByCanonical = new Map(frenchTechniques.entries.map((entry) => [entry.canonicalSlug, entry]));
for (const item of expected) {
  const frPath = item.fr || `/fr/techniques/${frenchTechniqueByCanonical.get(item.frCanonical)?.localizedSlug}/`;
  if (!frPath.includes("undefined")) {
    ...
    if (!enHtml.includes(`hreflang="es" href="${canonical}"`) || !enHtml.includes(`href="${item.path}"`)) throw new Error(`${item.path}: English equivalent lacks reciprocal Spanish discovery.`);
    const frTarget = frPath === "/fr/" ? "dist/fr/index.html" : join("dist", frPath.replace(/^\//, ""), "index.html");
    const frHtml = await readFile(frTarget, "utf8");
    if (!frHtml.includes(`hreflang="es" href="${canonical}"`) || !frHtml.includes(`href="${item.path}"`)) throw new Error(`${item.path}: French equivalent lacks reciprocal Spanish discovery.`);
  }
}
...
for (const phrase of ["Un sesgo es una lente de comprobación", `${SITE}/es/tecnicas/`, `${SITE}/es/habilidades/`]) if (!llms.includes(phrase)) throw new Error(`Spanish agent routing is missing required guidance: ${phrase}`);
