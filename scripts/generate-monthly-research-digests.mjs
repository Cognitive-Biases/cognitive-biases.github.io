...
manifest.counts = { ...(manifest.counts || {}), monthlyResearchDigests: digests.length };
manifest.files = { ...(manifest.files || {}), monthlyResearchDigests: `${SITE}/data/monthly-research-digests.json` };
await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

console.log(`Generated ${digests.length} monthly research digest(s).`);
