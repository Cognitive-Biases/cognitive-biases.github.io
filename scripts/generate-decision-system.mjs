...
const brandImage = (size, alt = "") => `<picture><source srcset="/assets/brand.webp" type="image/webp"><img src="/assets/biases_icon.png" width="${size}" height="${size}" alt="${esc(alt)}"></picture>`;
...
for (const situation of situationsData.situations) {
  const lensCards = situation.biases.map((slug) => card("Evidence-linked lens", biasTitle(slug), "Use this concept as a question to test the decision, not as a label for a person.", `/biases/${slug}/`)).join("");
  const techniqueCards = situation.techniques.map((slug) => {
    const technique = techniqueBySlug.get(slug);
    return card("Technique", technique.title, technique.purpose, `/techniques/${slug}/`);
  }).join("");
  const skill = skillBySlug.get(situation.skill);
  const body = `<section class="page-hero"><p class="eyebrow">Decision situation</p><h1>${esc(situation.title)}</h1><p class="lede">${esc(situation.summary)}</p><p><a class="button" href="/tools/decision-audit/">Use Decision Audit</a></p></section><section class="section"><p class="kicker">Signals to notice</p><h2>Reasons to slow down and check.</h2>${list(situation.signals)}</section><section class="section section--ink"><p class="kicker">Questions before action</p><h2>Make the missing evidence visible.</h2>${list(situation.questions)}</section><section class="section"><p class="kicker">Relevant lenses</p><h2>Concepts that may help you test the reasoning.</h2><div class="application-grid">${lensCards}</div></section><section class="section"><p class="kicker">Practical moves</p><h2>Do something different, not only notice a label.</h2><div class="application-grid">${techniqueCards}</div></section><section class="section"><p class="kicker">Skill to develop</p><h2><a href="/skills/${skill.slug}/">${esc(skill.title)}</a></h2><p class="lede">${esc(skill.outcome)}</p></section>`;
  await emit(`/situations/${situation.slug}/`, page(`${situation.title} Decision Review | Cognitive Biases`, `${situation.summary} Practical questions, relevant cognitive-bias lenses and decision techniques.`, `/situations/${situation.slug}/`, body));
}
...
for (const technique of techniquesData.techniques) {
  const relatedSituations = situationsData.situations.filter((s) => s.techniques.includes(technique.slug));
  const situationCards = relatedSituations.map((s) => card("Use in", s.title, s.summary, `/situations/${s.slug}/`)).join("");
  const lensCards = technique.biases.map((slug) => card("Related lens", biasTitle(slug), "Read the evidence and limits before treating this technique as relevant.", `/biases/${slug}/`)).join("");
  const body = `<section class="page-hero"><p class="eyebrow">Decision technique</p><h1>${esc(technique.title)}</h1><p class="lede">${esc(technique.purpose)}</p></section><section class="section"><p class="kicker">When to use</p><h2>A specific reasoning move for a specific problem.</h2><p class="lede">${esc(technique.whenToUse)}</p></section><section class="section section--ink"><p class="kicker">Procedure</p><h2>Run the technique.</h2><ol>${technique.steps.map((step) => `<li>${esc(step)}</li>`).join("")}</ol></section><section class="section"><p class="kicker">Limit</p><h2>What this technique cannot guarantee.</h2><p class="lede">${esc(technique.limitations)}</p></section><section class="section"><p class="kicker">Situations</p><h2>Where this may be useful.</h2><div class="application-grid">${situationCards}</div></section><section class="section"><p class="kicker">Evidence-linked concepts</p><h2>Why this check may matter.</h2><div class="application-grid">${lensCards}</div></section>`;
  await emit(`/techniques/${technique.slug}/`, page(`${technique.title} | Cognitive Biases`, `${technique.purpose} Step-by-step procedure, limitations and related decision situations.`, `/techniques/${technique.slug}/`, body, "HowTo"));
}
...
for (const path of await walk(OUT)) {
  ...
  const before = html;
  if (!html.includes('href="/decide/"')) html = html.replace('<nav aria-label="Primary">', '<nav aria-label="Primary"><a href="/decide/">Decide</a>');
...
for (const route of routes) {
  ...
  if (!sitemap.includes(`<loc>${loc}</loc>`)) sitemap = sitemap.replace("</urlset>", `<url><loc>${loc}</loc><lastmod>${situationsData.updatedAt}</lastmod></url></urlset>`);
}
