...
await emit("/data/", page({
  ...
  body: `<h2>What is available</h2><p>The public release includes ${biases.length} published concepts, ${evidence.length} evidence reviews, ${countEntries(contexts)} decision contexts and ${countEntries(comparisons)} reviewed comparisons. The files use the same names and links as the website so a tool can move from a record to the page a person can read.</p><h2>For assistants and agents</h2><p>An assistant can use the data to find a relevant bias, compare related ideas, show the reviewed evidence and point a person back to the source page. Older entries are not automatically treated as verified. Review status remains part of the record.</p><h2>For researchers and builders</h2><p>You can use the release for search, educational tools, experiments and integrations that fit the licence. Real reuse helps us decide which parts of the library deserve more attention.</p><h2>Download</h2><p><a href="/data/biases.json">Bias library</a><br><a href="/data/evidence.json">Evidence reviews</a><br><a href="/data/contexts.json">Decision contexts</a><br><a href="/data/comparisons.json">Comparisons</a><br><a href="/data/research-notes.json">Research notes</a><br><a href="/data/manifest.json">Release information</a></p><h2>Licence</h2><p>The current public content follows the licence in the repository. Attribution and non-commercial reuse conditions still apply. Please check the licence before redistributing or adapting the material.</p>`
}));
...
await writeFile(join(OUT, "data", "manifest.json"), `${JSON.stringify({name:"Cognitive Biases public knowledge release",version:1,generatedAt:TODAY,website:SITE,licence:"CC BY-NC-SA 4.0",counts:{concepts:biases.length,evidenceReviews:evidence.length,comparisons:countEntries(comparisons),contexts:countEntries(contexts)},files:{concepts:`${SITE}/data/biases.json`,evidence:`${SITE}/data/evidence.json`,contexts:`${SITE}/data/contexts.json`,comparisons:`${SITE}/data/comparisons.json`}}, null, 2)}\n`);
