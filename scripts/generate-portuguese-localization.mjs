...
function biasPath(entry) { return `/pt-br/vieses/${entry.localizedSlug}/`; }
function englishBiasPath(entry) { return `/biases/${entry.canonicalId}/`; }
function techniquePath(entry) { return `/pt-br/tecnicas/${entry.localizedSlug}/`; }
function skillPath(entry) { return `/pt-br/habilidades/${entry.localizedSlug}/`; }
function agentSkillPath(entry) { return `/pt-br/agent-skills/${entry.canonicalSlug}/`; }
...
function validateParity() {
  for (const entry of techniques.entries) if (!canonicalTechniqueBySlug.has(entry.canonicalSlug)) throw new Error(`Unknown canonical technique in pt-BR: ${entry.canonicalSlug}`);
  ...
  for (const entry of skills.entries) if (!canonicalSkillBySlug.has(entry.canonicalSlug)) throw new Error(`Unknown canonical Decision Skill in pt-BR: ${entry.canonicalSlug}`);
  ...
  for (const entry of agentSkills.entries) if (!canonicalAgentByName.has(entry.canonicalSlug)) throw new Error(`Unknown canonical Agent Skill in pt-BR: ${entry.canonicalSlug}`);
  ...
  for (const entry of translations.entries) for (const slug of entry.techniqueSlugs) if (!localizedTechniqueByCanonical.has(slug)) throw new Error(`${entry.canonicalId}: unknown linked technique ${slug}`);
}
...
function renderPage({ route, englishRoute, title, description, body, schemas = [] }) {
  ...
  const webPage = { "@context": "https://schema.org", "@type": "WebPage", "@id": `${canonical}#webpage`, name: title, description, url: canonical, inLanguage: "pt-BR", isPartOf: { "@type": "WebSite", "@id": `${SITE}/#website`, name: "Cognitive Biases", url: `${SITE}/` } };
...
async function generateBiasPages() {
  for (const entry of translations.entries) {
    ...
    const evidence = review ? `<section class="pt-section"><div class="pt-shell pt-prose"><p class="pt-kicker">${esc(ui.labels.evidence)}</p><h2>O que a revisão canônica permite afirmar.</h2><p><strong>Status canônico:</strong> <span lang="en">${esc(review.evidenceStatus)}</span></p><p>${esc(entry.evidenceSummary || review.summary || "Há uma revisão de evidências ligada a este conceito.")}</p><ol>${(review.sources || []).map((source) => `<li><a href="${esc(source.url)}" rel="external noreferrer">${esc(source.title)}</a>${source.year ? ` · ${esc(source.year)}` : ""}</li>`).join("")}</ol><p class="pt-muted">Revisão canônica: ${esc(review.reviewedAt || "data não informada")}.</p></div></section>` : `<section class="pt-section"><div class="pt-shell pt-prose"><p class="pt-kicker">${esc(ui.labels.evidence)}</p><h2>Revisão dedicada ainda não disponível.</h2><div class="pt-note"><strong>Tradução revisada ≠ evidência científica revisada.</strong><p>Esta página localiza o conceito canônico atual, mas nenhuma revisão de evidências dedicada está ligada a ele. Não use o rótulo como diagnóstico nem como causa certa de um caso individual.</p></div></div></section>`;
    const techniqueCards = linked.map((technique) => `<article class="pt-card"><h3><a href="${techniquePath(technique)}">${esc(technique.title)}</a></h3><p>${esc(technique.summary)}</p></article>`).join("");
...
async function generateTechniques() {
  ...
  for (const entry of techniques.entries) {
    ...
    const pageBody = `<article><section class="pt-hero pt-hero--compact"><div class="pt-shell"><p class="pt-kicker">Técnica de decisão</p><h1>${esc(entry.title)}</h1><p class="pt-lede">${esc(entry.summary)}</p></div></section><section class="pt-section"><div class="pt-shell pt-prose"><h2>Como aplicar</h2><ol>${entry.steps.map((step) => `<li>${esc(step)}</li>`).join("")}</ol><h2>Exemplo</h2><p>${esc(entry.example)}</p><h2>${esc(ui.labels.limits)}</h2><p>${esc(entry.boundary)}</p><p><a href="/techniques/${entry.canonicalSlug}/" hreflang="en" lang="en">Ver técnica canônica em inglês →</a></p></div></section></article>`;
    await emit(techniquePath(entry), renderPage({ route: techniquePath(entry), englishRoute: `/techniques/${entry.canonicalSlug}/`, title: `${entry.title}: como aplicar | Cognitive Biases`, description: `${entry.summary} Passos, exemplo e limites para usar a técnica em decisões reais.`, body: pageBody, schemas: [schema] }));
  }
}
async function generateSkills() {
  ...
  for (const entry of skills.entries) {
    ...
    const pageBody = `<article><section class="pt-hero pt-hero--compact"><div class="pt-shell"><p class="pt-kicker">Habilidade de decisão</p><h1>${esc(entry.title)}</h1><p class="pt-lede">${esc(entry.summary)}</p><p class="pt-question">${esc(entry.outcome)}</p></div></section><section class="pt-section"><div class="pt-shell pt-prose"><h2>${esc(ui.labels.whenToUse)}</h2><ul>${entry.whenToUse.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><h2>${esc(ui.labels.actions)}</h2><ol>${entry.actions.map((item) => `<li>${esc(item)}</li>`).join("")}</ol><h2>Exemplo</h2><p>${esc(entry.example)}</p><h2>${esc(ui.labels.limits)}</h2><p>${esc(entry.boundary)}</p><p><a href="/skills/${entry.canonicalSlug}/" hreflang="en" lang="en">Ver skill canônica em inglês →</a></p></div></section></article>`;
    await emit(skillPath(entry), renderPage({ route: skillPath(entry), englishRoute: `/skills/${entry.canonicalSlug}/`, title: `${entry.title} | Cognitive Biases`, description: `${entry.summary} Quando usar, como praticar, exemplo e limites.`, body: pageBody, schemas: [schema] }));
  }
}
async function generateAgentSkills() {
  ...
  for (const entry of agentSkills.entries) {
    ...
    const pageBody = `<article><section class="pt-hero pt-hero--compact"><div class="pt-shell"><p class="pt-kicker">Agent Skill · <code>${esc(entry.canonicalSlug)}</code></p><h1>${esc(entry.name)}</h1><p class="pt-lede">${esc(entry.summary)}</p></div></section><section class="pt-section"><div class="pt-shell pt-prose"><h2>Use quando</h2><ul>${entry.useWhen.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><h2>Procedimento</h2><ol>${entry.procedure.map((item) => `<li>${esc(item)}</li>`).join("")}</ol><h2>Formato da resposta</h2><ul>${entry.output.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><h2>Guardrails</h2><ul>${entry.guardrails.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><p><a href="/pt-br/agent-skills/${entry.canonicalSlug}/SKILL.md">Abrir SKILL.md em pt-BR →</a></p><p><a href="/agent-skills/${entry.canonicalSlug}/" hreflang="en" lang="en">Abrir Agent Skill canônica →</a></p></div></section></article>`;
    await emit(agentSkillPath(entry), renderPage({ route: agentSkillPath(entry), englishRoute: `/agent-skills/${entry.canonicalSlug}/`, title: `${entry.name} — Agent Skill em pt-BR | Cognitive Biases`, description: entry.summary, body: pageBody }));
  ...
  for (const entry of translations.entries) {
    const dir = join(OUT, "pt-br", "agent-skills", `bias-${entry.canonicalId}`);
    await mkdir(dir, { recursive: true });
...
async function generateMachineData() {
  ...
  const localizedBiases = translations.entries.map((entry) => {
    const review = evidenceReviews.get(entry.canonicalId);
    return { canonicalId: entry.canonicalId, locale: "pt-BR", localizedSlug: entry.localizedSlug, localizedLabel: entry.localizedLabel, englishLabel: entry.englishLabel, aliases: entry.aliases, searchTerms: entry.searchTerms, summary: entry.summary, practicalQuestion: entry.practicalQuestion, examples: entry.examples, boundary: entry.boundary, techniqueSlugs: entry.techniqueSlugs, translationReview: { status: entry.state, reviewedAt: entry.reviewedAt, sourceRelease: entry.sourceRelease }, canonicalEvidence: review ? { status: review.evidenceStatus, reviewedAt: review.reviewedAt, summary: review.summary, sources: review.sources } : null, url: url(biasPath(entry)), canonicalUrl: url(englishBiasPath(entry)) };
  });
  const workflowSkills = agentSkills.entries.map((entry) => ({ ...entry, locale: "pt-BR", canonical: canonicalAgentByName.get(entry.canonicalSlug) || null, skillUrl: url(agentSkillPath(entry)), skillMarkdown: url(`/pt-br/agent-skills/${entry.canonicalSlug}/SKILL.md`) }));
  const biasSkills = translations.entries.map((entry) => ({ name: `bias-${entry.canonicalId}`, locale: "pt-BR", canonicalBias: entry.canonicalId, title: entry.localizedLabel, skillMarkdown: url(`/pt-br/agent-skills/bias-${entry.canonicalId}/SKILL.md`), humanPage: url(biasPath(entry)) }));
  const manifest = { version: 1, locale: "pt-BR", canonicalLocale: "en", status: "reviewed-partial-human-layer", sourceRelease: release.releaseVersion, generatedAt: TODAY, canonicalSite: `${SITE}/`, localizedSite: `${SITE}/pt-br/`, coverage: { concepts: translations.entries.length, canonicalPublishedConcepts: publishedBiases.length, conceptPolicy: "curated-reviewed-subset-no-silent-fallback", techniques: techniques.entries.length, canonicalTechniques: canonicalTechniqueBySlug.size, decisionSkills: skills.entries.length, canonicalDecisionSkills: canonicalSkillBySlug.size, workflowAgentSkills: agentSkills.entries.length, canonicalWorkflowAgentSkills: canonicalAgentByName.size, localizedBiasAgentSkills: biasSkills.length }, semantics: { canonicalIdentifiersRemainEnglish: true, evidenceStatusRemainsCanonical: true, translationReviewSeparateFromEvidenceReview: true, sourceTitlesRemainOriginal: true, translateJsonFieldNames: false, noEnglishFallbackPresentedAsReviewedPortuguese: true }, datasets: { concepts: `${SITE}/pt-br/data/biases.json`, techniques: `${SITE}/pt-br/data/techniques.json`, decisionSkills: `${SITE}/pt-br/data/skills.json`, agentSkills: `${SITE}/pt-br/data/agent-skills.json`, search: `${SITE}/pt-br/data/search.json`, localeRegistry: `${SITE}/data/locales.json`, aiLocaleRegistry: `${SITE}/ai/locales.json` }, agentRouting: `${SITE}/pt-br/llms.txt` };
  ...
  await writeFile(join(dir, "agent-skills.json"), JSON.stringify({ version: 1, locale: "pt-BR", workflowSkills, biasSkills }, null, 2) + "\n");
...
async function updateSitemap(paths) {
  ...
  for (const path of paths) { const loc = url(path); if (!sitemap.includes(`<loc>${loc}</loc>`)) sitemap = sitemap.replace("</urlset>", `<url><loc>${loc}</loc></url></urlset>`); }
  await writeFile(sitemapPath, sitemap);
  await writeFile(join(OUT, "pt-br", "sitemap.xml"), `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${paths.map((path) => `<url><loc>${url(path)}</loc></url>`).join("")}</urlset>\n`);
}
async function patchEnglishDiscovery() {
  const pairs = [["/", "/pt-br/"], ["/explore/", "/pt-br/explorar/"], ["/techniques/", "/pt-br/tecnicas/"], ["/skills/", "/pt-br/habilidades/"], ["/agent-skills/", "/pt-br/agent-skills/"], ...translations.entries.map((entry) => [englishBiasPath(entry), biasPath(entry)]), ...techniques.entries.map((entry) => [`/techniques/${entry.canonicalSlug}/`, techniquePath(entry)]), ...skills.entries.map((entry) => [`/skills/${entry.canonicalSlug}/`, skillPath(entry)]), ...agentSkills.entries.map((entry) => [`/agent-skills/${entry.canonicalSlug}/`, agentSkillPath(entry)])];
...
async function patchEnglishPage(englishRoute, portugueseRoute) {
  ...
  let html; try { html = await readFile(path, "utf8"); } catch { return; }
  if (!html.includes('hreflang="pt-BR"')) html = html.replace("</head>", `<link rel="alternate" hreflang="pt-BR" href="${url(portugueseRoute)}"></head>`);
