...
for (const command of ["check:de", "check:french-localization", "check:french-skills", "check:portuguese-localization", "check:localization", "check:localization-impact"]) expect(Boolean(pkg.scripts?.[command]), `package.json must expose ${command}`);
for (const command of ["check:de", "check:french-localization", "check:french-skills", "check:portuguese-localization", "check:localization"]) expect(pkg.scripts?.check?.includes(command), `npm run check must include ${command}`);
