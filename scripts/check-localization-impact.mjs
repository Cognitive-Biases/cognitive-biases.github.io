...
for (const rule of profile.impactRules || []) {
  const files = changed.filter((file) => rule.paths.some((pattern) => matchesGlob(file, pattern))).filter((file) => !isLocaleSpecificSource(file));
...
function isLocaleSpecificSource(file) {
  return /^data\/(?:de|ru)\//.test(file) || /(?:-fr|-es|pt-br|french|spanish|portuguese|german|russian|\/(?:de|ru|fr|es|pt-br)\/)/i.test(file) || /^docs\/(?:GERMAN|RUSSIAN)_LOCALIZATION\.md$/.test(file) || /^docs\/localization-(?:fr|es|pt-br)\.md$/.test(file);
}
function isLocaleReviewFile(file, locale) {
  const lower = `/${file.toLowerCase()}`;
  const tokens = locale === "pt-BR" ? ["pt-br", "portuguese"] : locale === "fr" ? ["-fr", "/fr/", "french", "glossary-fr", "localization-fr"] : locale === "es" ? ["-es", "/es/", "spanish", "glossary-es", "localization-es"] : locale === "de" ? ["/de/", "german", "check-de", "generate-de", "finalize-de"] : locale === "ru" ? ["/ru/", "russian", "check-ru", "generate-ru", "finalize-ru"] : [locale.toLowerCase()];
  return tokens.some((token) => lower.includes(token));
}
function isHighRiskCanonicalFile(file) {
  return /^data\/(?:biases|techniques|agent-skills|everyday-guides)\.json$/.test(file) || /^data\/skills\/.+\.json$/.test(file) || file === "index.html" || file === "llms.txt" || /^assets\//.test(file) || /^public\//.test(file) || /^ai\//.test(file) || (/^skills\//.test(file) && !file.startsWith("skills/translation-review/")) || /^scripts\/(?:build|generate|enhance|apply|finalize)-.+\.mjs$/.test(file);
}
