...
7. `.arwp/README.md`, `.arwp/site-focus.json`, `.arwp/technical-seo-critic.json`, `.arwp/image-discovery.json` and `.arwp/internal-discovery.json` — current site-focus boundary, second-pass technical Search review, image-discovery review, internal-discovery/distribution review and Search/AI verification policy when navigation, URLs, discovery, public structure, visual assets or agent surfaces are involved.
...
| Site/search/discovery | human source and build code | sitemap, metadata, ARWP/search surfaces, internal links, canonical URLs, Technical SEO Critic, Image Discovery, Internal Discovery & Distribution | `npm run build`, `npm run check`, `npm run check:technical-seo-critic`, `npm run check:image-discovery`, `npm run check:internal-discovery`; review post-deploy live evidence |
...
| Internal links/page utilities | `data/relations-v2.json`, `data/contexts.json`, `data/comparisons-*.json`, evidence-review state and generator/post-processor code | visible/schema breadcrumbs, canonical continuation targets, reverse links, `public/internal-discovery.js`, `.arwp/internal-discovery.json` | `npm run check:internal-discovery`; live bounded check after deployment |
...
- For internal discovery, reuse canonical reviewed relation/context/comparison data instead of manufacturing keyword relationships. A relationship may be visible only when it helps the user journey; do not expose every graph edge merely to raise link counts.
...
- General ARWP validation/Growth is pinned to reviewed revision `793483e3404a97f7892e86bcda3fd317d5c7427c`. The completed Technical SEO Critic review remains pinned to its reviewed source revision `28c9cc9b75fa2b17791b7b294b4249fa28496445`, the completed Image Discovery review remains pinned to `73bd2a64e2e746deeb8de792ca01f654f0a7d33a`, and Internal Discovery & Distribution is pinned to `793483e3404a97f7892e86bcda3fd317d5c7427c`. The Site Focus v0.3 contract and its dedicated workflow may remain on their separately reviewed compatible revision. Never silently follow upstream `main`; document revision boundaries in `.arwp/README.md`.
...
- Apply Internal Discovery & Distribution only after the canonical sitemap and reviewed relation/context/comparison layers exist. `scripts/apply-internal-discovery-distribution.mjs` may improve visible/schema breadcrumbs, add a compact utility bar and generate data-aware continuation only where curated inputs provide a real next step. `scripts/check-internal-discovery-distribution.mjs` verifies the final graph contract; `scripts/check-live-internal-discovery-distribution.mjs` checks a bounded live cohort after deployment.
- Share, Save, Copy link and Cite are distribution/user utilities, not ranking factors. Copy/share/citation must use the canonical URL. Save is browser-local and must not imply account sync. Do not publish a Preferred Sources control until site applicability is verified in Google's source-preferences tool.
- Do not invent a magic internal-link count, PageRank-like site score or universal click-depth rule. Preserve partial/unknown graph evidence and fail CI only on deterministic contracts such as broken canonical targets or missing required reciprocal links.
...
- Site Focus, Site Readiness Gate, Technical Integrity, Technical SEO Critic, Image Discovery and Internal Discovery & Distribution are bounded implementation checks. A pass is not evidence of ranking, indexing, Google Images inclusion, Discover placement, AI citation, shares, return visits, traffic, educational effect or business impact.
...
npm run check:internal-discovery
```
...
For public Search/AI structure, inspect the post-deploy `AWRP Site Quality` artifact plus the live Technical SEO Critic, Image Discovery and Internal Discovery & Distribution checks after Pages deploys the target revision. Treat P0 findings as review/fix signals and preserve the raw reports.
