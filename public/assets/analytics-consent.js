(function () {
  ...
  var copy = {
    ...
    de: {
      ...
      message: "Wir verwenden optionale Analysedaten, um diese Website zu verbessern. ",
      allow: "Erlauben",
      decline: "Ablehnen"
    },
    ru: {
      label: "Настройки аналитики",
      message: "Мы используем необязательную аналитику, чтобы улучшать сайт. ",
      allow: "Разрешить",
      decline: "Отклонить"
    },
    fr: {
      label: "Paramètres d’analyse",
      message: "Nous utilisons des données d’analyse facultatives pour améliorer ce site. ",
      allow: "Autoriser",
      decline: "Refuser"
    },
    "pt-br": {
      label: "Configurações de análise",
      message: "Usamos dados de análise opcionais para melhorar este site. ",
      allow: "Permitir",
      decline: "Recusar"
    },
    es: {
      label: "Configuración de analítica",
      message: "Usamos datos de analítica opcionales para mejorar este sitio. ",
      allow: "Permitir",
      decline: "Rechazar"
    },
    it: {
      label: "Impostazioni di analisi",
      message: "Usiamo dati di analisi facoltativi per migliorare questo sito. ",
      allow: "Consenti",
      decline: "Rifiuta"
    }
  };
  ...
  function prompt() {
    ...
    box.setAttribute("data-analytics-consent", "");
    ...
    box.addEventListener("click", function (event) {
      if (event.target.matches("[data-analytics-yes]")) {
        localStorage.setItem(key, "yes");
        box.remove();
        load();
      }
      if (event.target.matches("[data-analytics-no]")) {
