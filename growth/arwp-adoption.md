...
| `non-commodity-evidence` | The site already publishes reviewed evidence, research notes, decision contexts, comparisons, datasets and research deltas. | Keep original/evidence-backed material as the growth surface; do not create thin query-variant pages. | Existing evidence, research and search-quality checks. | Impressions/clicks and cited landing pages, without assuming causality. |
...
| `identity-and-provenance` | Canonical Organization identity, trust, corrections, citations and knowledge graph are active. | Keep one resolvable identity and evidence state across human and machine surfaces. | `scripts/check-arwp-layers.mjs` plus structured-data checks. | Citation quality and fewer ambiguous entity/source resolutions. |
...
| `freshness-without-fake-recency` | Bias/research resources use content-backed dates, but base static sitemap routes inherited the build date on every build. | Remove `lastmod` from static routes that have no reliable modification date; keep content-backed dates for dated resources. | `scripts/finalize-late-seo.mjs` now rewrites and asserts the policy during every site build. | Cleaner crawl/freshness semantics; no synthetic recency claim. |
| `platform-ai-measurement` | Google Search measurement exists; AI visibility must remain separately observed. | Run the current ARWP Growth audit after successful production deploys and weekly, storing both site and ARWP revisions. | `.github/workflows/arwp-growth.yml` artifact: `arwp-growth.json`, `site-revision.txt`, `arwp-revision.txt`. | Search, generative/AI citations, referrals and product outcomes remain separate metrics; missing data stays missing. |

Not selected: `preferred-source-loop` is not added merely because a platform feature exists. Agent-facing routes remain governed by the separate ARWP profile and are not represented here as ranking factors.
...
- Remaining external gates: Search Console/analytics windows, Discover/image data where available, and observable AI citation/referral evidence.
...
`keep` for the implementation baseline; longitudinal outcome evidence remains pending.

Reason: the rollout corrects a concrete synthetic-freshness issue, updates validation to the current ARWP baseline, and adds reproducible measurement plumbing while deliberately preserving existing crawler-training and licence boundaries.
