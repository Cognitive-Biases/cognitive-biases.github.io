...
- `growth:entity-sameas`: implemented using the real `Cognitive-Biases` GitHub organization profile; no synthetic profiles were created. The final site build currently does not retain a canonical `SoftwareApplication` JSON-LD node, so app identity is treated as optional rather than invented.
